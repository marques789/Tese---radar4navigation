\chapter{Robotic Navigation using ROS} \label{ch:BaseWork}
...
\begin{itemize}
  ...
  \end{cases}
    ...
    Where \textbf{path\_dist} is the distance from the endpoint of the trajectory to the global path in map cells, \textbf{goal\_dist} is the distance from the endpoint of the trajectory to the local goal in map cells, and \textbf{maxobscost} is equal to the maximum obstacle cost (given by the local costmap) of all the points along the trajectory.
    \end{itemize}
